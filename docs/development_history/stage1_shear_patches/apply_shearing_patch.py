...
old = """                ind_source, input, label = batch[0].to(device), batch[1].to(device), batch[2].to(device)

                disp, inter_lf, pred_lf = model(ind_source, input, opt)"""

new = """                ind_source, input, label = batch[0].to(device), batch[1].to(device), batch[2].to(device)
...
if 'apply_shear' in code:
    print("apply_shear already in train.py — skipping")
