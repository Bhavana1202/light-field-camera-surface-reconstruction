...
def main():
    ...
    if opt.arb_sample:
        ...
        model_dir = 'model_arbIn_{}_S{}_pr{}_ps{}_lr{}_step{}x{}_shear{}'.format(opt.dataset, opt.num_source, opt.psv_range, opt.psv_step, opt.lr, opt.step, opt.reduce, opt.shear_amount)
        opt.crop_size = 0

    else:
        opt.num_source = opt.angular_in * opt.angular_in
        model_dir = 'model_fixIn_{}_S{}_pr{}_ps{}_lr{}_step{}x{}_shear{}'.format(opt.dataset, opt.num_source, opt.psv_range, opt.psv_step, opt.lr,opt.step,opt.reduce, opt.shear_amount)
